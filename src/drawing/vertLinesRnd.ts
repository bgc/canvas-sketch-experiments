...
type rand = { value: () => number; noise2D: (x:number, y:number, frequency?:number, amplitude?:number) => number, pick: (arr: any[]) => any }
...
export const vertLinesRnd = (props: DrawingFNProps, rInstance: rand, grid: Grid) => {
  ...
  lines.forEach((rowLines: GridElement[]) => {
    ...
    col.forEach((point: GridElement, idx: number, arrrgh: any) => {
      ...
      if(idx >= arrrgh.length-3){
        ...
        const noise = rInstance.noise2D(currPoint.u, currPoint.v, 12, 2*Math.PI)
        const ctrlNoise = rInstance.noise2D(ctrlPoint.u, ctrlPoint.v, 12, 2*Math.PI)
        const endNoise = rInstance.noise2D(endPoint.u, endPoint.v, 12, 2*Math.PI)

        context.moveTo(currPoint.x + AMPLITUDE*Math.cos(noise), currPoint.y)
        context.quadraticCurveTo(ctrlPoint.x+ AMPLITUDE*Math.cos(ctrlNoise), ctrlPoint.y, endPoint.x+ AMPLITUDE*Math.cos(endNoise), endPoint.y)
      }
